  ...
  export default {
    ...
    computed:{
      ...
      payClass(){
        ...
        const {minPrice}=this.info//起送最小价格

        return totalPrice>=minPrice?'enough':'not-enough'
      },
      payText(){
        const {totalPrice}=this//总价格
        const {minPrice}=this.info//最小价格
        if(totalPrice===0){
          return `￥${minPrice}元起送`
      ...
      listShow(){
        ...
        if(this.totalCount==0){
          //解决2次点击增加，isshow仍为true，购物车会显示问题
          this.isShow=false
          return false
        }
        //购物车滑动效果
        if(this.isShow){
          this.$nextTick(()=>{
            if(!this.scroll){//如果不存在，再从新创建对象，保证betterscroll单例，否则会出现多次点击事件导致数量翻倍增加
              this.scroll=new BScroll('.list-content',{
                click:true
              })
            }else{
              this.scroll.refresh()//让滚动条刷新下，从新统计内容高度
            }
          })
        }
